...
const BUFF_CATS = {
    ...
    def: { label: "🛡 Défense", ids: ["def_15", "hp_max_15_cond", "res_lp_30", "dmg_resist_10", "dmg_resist_20_cond", "dmg_resist_80_cond", "dmg_resist_15_cond", "dmg_resist_20_team", "shield_hits_3_team", "shield_hits_5", "stealth_shield", "endurance_cond", "phoenix_revive_100", "dodge_3s", "protection_shield", "limit_shield", "dmg_down_provoked", "dmg_resist_aoe_30", "debuff_immune", "regen_team"] },
    ...
    dot: { label: "☠ DoT/Ennemi", ids: ["bleed", "dot_poison", "weakness_poison_enemy", "weak_point_detect", "weakness_element", "weakness_elem_team", "debuff_aoe_taken_10", "debuff_crit_taken_10", "debuff_crit_taken_15", "block_shield", "atk_nerf_20", "def_nerf_20", "def_nerf_20_cond", "resist_nerf_30", "crit_rate_nerf_20", "dot_dark_flame", "dot_love_disease", "dot_deep_bleed"] },
...
const BUFFS = {
    ...
    "debuff_crit_taken_15": { label: "Dég. Surcritique reçus +15% (ennemi)", color: "#dd6644", power: 18 },
...
const CRIT_SKILLS = {
    ...
    "Eludia": { name: "", desc: "Inflige 1 020% de la puissance d'attaque en dégâts critiques dans un rayon de 2m autour de la cible.\nEn cas d'attaque réussie, attribue l'effet de renforcement Augmentation de la précision de 20% aux alliés à distance pendant 2 sec. (Effet cumulable)\nEn cas d'attaque réussie, attribue l'effet de renforcement Augmentation du taux CRIT de 30% à soi-même pendant 2 sec.\nEn cas d'attaque critique, attribue l'effet cumulable Braise inextinguible à soi-même. À 2 cumuls, inflige des dégâts jusqu'à la mort à soi-même. (Suppression impossible en cas de dégâts mortels)", cd: 2, buffs: ["accuracy_20_range", "crit_rate_30_cond"], atk_pct: 1020, aoe: true },
};
...
const EXCLUSIVE_SKILLS = {
    ...
    "Eludia": { name: "Flambée purifiante", desc: "Inflige 1 570% de la puissance d'attaque en dégâts dans un rayon de 3m autour de la cible.\nEn cas d'attaque réussie, confère aux alliés l'effet de renforcement Augmentation des DGTS CRIT de 20% pendant 10 sec.\nEn cas de dégâts mortels, l'effet est amélioré et attribue l'effet de renforcement supplémentaire Autorité du phénix. (Effet cumulable)\nEn cas d'attaque réussie, attribue l'effet de résurrection Souffle de régénération 1 fois à soi-même. (Effet cumulable, se déclenche 1 seule fois par combat)", cd: 10, buffs: ["crit_dmg_team_20", "phoenix_revive_100"], atk_pct: 1570, aoe: true },
};
const MONSTERS = {
    ...
    "Eludia": { type: "range", awakenings: [AWK3, { level: 5, desc: "Début de combat :\n- Portée +3 (Suppression impossible, cumulable).\n- Résurrection Souffle de régénération 1 fois à soi-même.\nSur dégâts mortels :\n- Saccage du Phénix (Dégâts +25%/cible touchée, max 10).\n- Rayon Zone +30% (Suppression impossible).", buffs: ["range_3", "phoenix_revive_100", "dmg_aoe_25_scaling_cond", "aoe_range_30_cond"], pvp: true, pve: true }, { level: 7, desc: "Attaque réussie :\n- Dég. Surcritique reçus ennemi +15% / 10s.\nSur dégâts mortels :\n- ATQ +30%/cumul (max 150%, permanent).\n- Dégâts +20%.", buffs: ["debuff_crit_taken_15", "atk_30_stack_lethal_cond", "dmg_20"], pvp: true, pve: true }] },
};
